...
{
  puts("perf-test version " VERSION_STR);
}
...
int main(int argc, char *argv[])
{
  ...
  snd_rawmidi_params_t *pRParams;
  snd_rawmidi_params_t *pSParams;
  snd_rawmidi_params_malloc(&pRParams);
  snd_rawmidi_params_malloc(&pSParams);
  ...
  if (inputp)
  {
    if ((err = snd_rawmidi_params_set_buffer_size(input, pRParams, BUFSIZE)) < 0)
    {
      error("cannot set buffer size of %d : %s", BUFSIZE, snd_strerror(err));
      goto _exit;
    }
  ...
  if (send_data)
  {
    if ((err = snd_rawmidi_params_set_buffer_size(output, pSParams, BUFSIZE)) < 0)
    {
      error("cannot set buffer size of %d : %s", BUFSIZE, snd_strerror(err));
      goto _exit;
    }
    ...
    int transferred = 0;
    while (transferred != send_data_length)
    {
      err = snd_rawmidi_write(output, &(send_data[transferred]), send_data_length - transferred);
      if (err < 0)
      {
